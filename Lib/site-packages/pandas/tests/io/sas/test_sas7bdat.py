...
class TestSAS7BDAT:
    ...
    def test_from_file(self, dirpath, data_test_ix):
        ...
        for k in test_ix:
            fname = os.path.join(dirpath, f"earopean_leagues{k}.sas7bdat")
    ...
    def test_path_pathlib(self, dirpath, data_test_ix):
        df0, test_ix = data_test_ix
        for k in test_ix:
            fname = Path(os.path.join(dirpath, f"earopean_leagues{k}.sas7bdat"))
            df = pd.read_sas(fname, encoding="utf-8")
            tm.assert_frame_equal(df, df0)
    ...
    def test_path_localpath(self, dirpath, data_test_ix):
        ...
        df0, test_ix = data_test_ix
        for k in test_ix:
            fname = LocalPath(os.path.join(dirpath, f"earopean_leagues{k}.sas7bdat"))
            df = pd.read_sas(fname, encoding="utf-8")
            tm.assert_frame_equal(df, df0)
